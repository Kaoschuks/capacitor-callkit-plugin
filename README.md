...
5. **Managed mode (optional)** — `setup({ android: { selfManaged: false } })` uses the system dialer UI instead. Add `READ_PHONE_STATE`, `READ_PHONE_NUMBERS` and `CALL_PHONE` to your app manifest, request the `phone` permission, and have the user enable the calling account (`openPhoneAccountSettings()`).
